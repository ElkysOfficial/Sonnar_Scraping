...
### 1.1 Cache em disco local por `job_id` 🟢 considerado

PNG salvo em `assets/cache/<job_id>.png` na primeira renderização; próximas leem do disco.

- **Ganho estimado:** -90% CPU do formatter em pico (mesma vaga vira muitos deliveries).
- **RAM:** processo continua na VPS (teto 600MB inalterado).
- **Custo:** ~1,8GB de disco (100KB × 18k vagas).
- **Risco:** baixíssimo. Reversível (apaga cache, volta ao comportamento atual).
- **Esforço:** ~1 dia.

### 1.2 Migrar para `@vercel/og` 🟢 considerado

Função no portal Vercel renderiza o card; CDN cacheia por URL. Sender aponta para URL pública em vez de `localhost:3001`.

- **Ganho:** processo `sonnar-wa-formatter` **sai da VPS** (-500MB RAM teto, -1 processo PM2).
- **Custo:** latência primeira render +200–400ms (cacheado depois). Edge time grátis no tier free da Vercel.
- **Risco:** rollback exige reverter integração no sender.
- **Esforço:** 1–2 dias.
