...
describe("Logger Functions", () => {
  ...
  describe("sayLog", () => {
    it("should call console.log with formatted message", () => {
      ...
      assert.strictEqual(args[0], "\x1b[36m[Sonar Bot | TALK]\x1b[0m")
  ...
  describe("inputLog", () => {
    it("should call console.log with input color", () => {
      ...
      assert.strictEqual(consoleLogMock.mock.calls.length, 1)
      const args = consoleLogMock.mock.calls[0].arguments
      assert.strictEqual(args[0], "\x1b[30m[Sonar Bot | INPUT]\x1b[0m")
  ...
  describe("infoLog", () => {
    it("should call console.log with blue color", () => {
      ...
      assert.strictEqual(consoleLogMock.mock.calls.length, 1)
      const args = consoleLogMock.mock.calls[0].arguments
      assert.strictEqual(args[0], "\x1b[34m[Sonar Bot | INFO]\x1b[0m")
  ...
  describe("successLog", () => {
    it("should call console.log with green color", () => {
      ...
      assert.strictEqual(consoleLogMock.mock.calls.length, 1)
      const args = consoleLogMock.mock.calls[0].arguments
      assert.strictEqual(args[0], "\x1b[32m[Sonar Bot | SUCCESS]\x1b[0m")
  ...
  describe("errorLog", () => {
    it("should call console.log with red color", () => {
      ...
      assert.strictEqual(consoleLogMock.mock.calls.length, 1)
      const args = consoleLogMock.mock.calls[0].arguments
      assert.strictEqual(args[0], "\x1b[31m[Sonar Bot | ERROR]\x1b[0m")
  ...
  describe("warningLog", () => {
    it("should call console.log with yellow color", () => {
      ...
      assert.strictEqual(consoleLogMock.mock.calls.length, 1)
      const args = consoleLogMock.mock.calls[0].arguments
      assert.strictEqual(args[0], "\x1b[33m[Sonar Bot | WARNING]\x1b[0m")
  ...
  describe("ANSI Colors", () => {
    it("all functions should reset color at the end", () => {
      ...
      for (const { fn, name } of logFunctions) {
        ...
        assert.ok(colorCode.includes("\x1b[0m"), `${name} should include color reset code`)
      }
    })
  ...
  describe("Prefixes", () => {
    it("all logs should have prefix [Sonar Bot]", () => {
      const logFunctions = [logger.sayLog, logger.inputLog, logger.infoLog, logger.successLog, logger.errorLog, logger.warningLog]
      ...
      for (const fn of logFunctions) {
        ...
        assert.ok(prefix.includes("Sonar Bot"), "Should include 'Sonar Bot' in prefix")
      }
    })
