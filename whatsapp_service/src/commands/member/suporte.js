...
Você também pode escrever o texto e responder a mensagem com o comando ${PREFIX}suporte`,
   ...
   */
  handle: async ({ fullArgs, args, sendReply, sendWaitReply, sendReact, replyText, isImage, isVideo, isAudio, downloadImage, webMessage }) => {
    if (!OPENAI_API_KEY) {
      throw new WarningError("O suporte inteligente não está disponível no momento. Entre em contato com o administrador do bot!")
    }

    if (isVideo) {
      throw new WarningError("Não consigo interpretar vídeos ainda! Envie uma imagem ou texto!")
    }

    if (isAudio) {
      throw new WarningError("Não consigo interpretar áudios ainda! Envie uma imagem ou texto!")
    }
...
- Envie uma imagem com ${PREFIX}suporte para análise visual`
    ...
    const finalText = doubleContext ? `Contexto anterior: ${replyText}\n\nNova questão: ${text}` : text
    ...
    if (finalText) {
      ...
      if (finalText.length < minLength) {
        throw new DangerError(`O texto deve ter no mínimo ${minLength} caracteres.`)
      }

      if (finalText.length > maxLength) {
        throw new DangerError(`O texto deve ter no máximo ${maxLength} caracteres.`)
      }
    }
    ...
    const messages = [
      {
        role: "system",
        content: `Você é um assistente especializado em suporte técnico do Sonar Bot.

Responda apenas assuntos relacionados a: tecnologia, programação, desenvolvimento de bots, inteligência artificial, 
machine learning, ou assuntos relacionados ao Sonar Bot.

Responda apenas em português do Brasil.
Seja direto e objetivo nas respostas, salvo se o usuário solicitar explicações mais aprofundadas.

Quando receber imagens, analise o conteúdo visual primeiro e interprete-o considerando o contexto técnico do Sonar Bot.
...
Se alguém te pedir o link de alguma Host, envie!`
    ...
    messages.push({
      role: "system",
      content: fs.readFileSync(path.resolve(__dirname, "..", "..", "..", "CLAUDE.md"), "utf-8")
    })
