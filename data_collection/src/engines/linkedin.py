...
async def get_linkedin_jobs() -> list:
    ...
    jobs = []
    max_pages = int(os.getenv("LINKEDIN_MAX_PAGES", "20"))
    max_empty_pages = int(os.getenv("LINKEDIN_MAX_EMPTY_PAGES", "1"))
    for stack in stacks:
        page_index = 0
        empty_pages = 0
        while page_index < max_pages and empty_pages < max_empty_pages:
            start = page_index * 10
            async with httpx.AsyncClient() as client:
                response = await client.get(
                    f'https://br.linkedin.com/jobs/api/seeMoreJobPostings/search?keywords={stack}&location=Brasil&geoId=106057199&start={start}'
                )
